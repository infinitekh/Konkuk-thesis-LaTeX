...
\titles[korean]{국문 논문 제목}
\titles[english]{영문 논문 제목}
\titles[subtitle]{한글논문 부제목}
% 영어 부제목은 고려하지 않았음. 필요하시면 수정하세요.
% 
\author[korean]{김 혁}
\author[hanja]{金 爀}
\author[english]{KIM, Hyeok}
% 청구논문과 양쪽 정렬이 되어야 하므로 글자 사이사이에 \hfill을 집어 넣으시고,
% 교수님 성함이 4글자가 넘어가면 어케하는지 학교에 문의하세요.
\advisor{김\hfill강}{Prof. KIM}{}
\department[physics]
% 현재 가능한 리스트  
% physics  chemistry 
% 어렵지 않으니 konkukthesis.cls 파일 안에서 department 검색해서 보시고 본인과에 대해 작성하세요.
% 
\referee[1]{여두령}
\referee[2]{송차령}
\referee[3]{김지도}
...
\back
\chapter{부록}
%부록은  1 2 3 4 5 를 적는데, 틀을 만드는건 바보짓. 그냥 직접 쓰시오.
%건국대학교 서식은  [부록1.] 이런식으로 부록을 나눔. 직접 입력하는게 편함.
%부록이 크면 A,B,C 권으로 나누는것도 있는거 같은데, 그건 스스로.
\section{[부록1.] 자료}
\subsection{서브섹션}
